...
public class RouteCapableUnitOfWorkRequestDispatcher implements RequestDispatcher {
    ...
    public RouteCapableUnitOfWorkRequestDispatcher(final UnitOfWork unitOfWork, final RequestDispatcher dispatcher,
    ...
    /**
     * Retrieves the route key header ('RouteKey') and returns the corresponding {@link SessionFactory}.
     * 
     * @param context
    ...
    private SessionFactory route(final HttpContext context) {
        final String routeKey = context.getRequest().getHeaderValue(ROUTE_KEY_HEADER_NAME);
        final SessionFactory factory = sessionFactoryMap.get(Optional.fromNullable(routeKey));
        if (null == factory) {
            throw new NotFoundException("No SessionFactory found for RouteKey[" + routeKey + "]");
        }
